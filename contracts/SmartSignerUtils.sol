...
contract ECDSAWalletVerifier {
    ...
    function publicKeyToAddress(
        Secp256k1.Point memory publicKey
    ) public pure returns (address) {
        bytes32 publicKeyHash = keccak256(
    ...
    function PubDerive(
        ...
        uint256 nonce
    ) public pure returns (uint256 pubkeyX, uint256 pubkeyY) {
        (uint256 px, uint256 py) = Secp256k1.ecMul(
            uint256(nonce),
            Secp256k1.Gx,
            Secp256k1.Gy,
            Secp256k1.A,
        ...
        (pubkeyX, pubkeyY) = Secp256k1.ecAdd(
            pubkey[0],
            pubkey[1],
            px,
            py,
    ...
    function PrivDerive(
        ...
        return
            bytes32(addmod(uint256(secret_key), uint256(nonce), Secp256k1.PP));
    }

    function SharedSecret(
        uint256 my_secret,
        uint256[2] memory their_public
    ) public pure returns (uint256 xPX, uint256 xPY) {
        (xPX, xPY) = Secp256k1.ecMul(
            uint256(my_secret),
            their_public[0],
            their_public[1],
            Secp256k1.A,
    ...
    function CreateProof(
        ...
        public
        view
        returns (uint256 pubkeyX, uint256 pubkeyY, uint256 out_e, uint256 out_s)
    {
        (pubkeyX, pubkeyY) = Secp256k1.ecMul(
            secret % Secp256k1.PP,
            Secp256k1.Gx,
            Secp256k1.Gy,
            Secp256k1.A,
        ...
        uint256 k = log256(
            uint256(keccak256(abi.encodePacked(message, secret))) % Secp256k1.PP
        ) * block.timestamp;
        (uint256 kgX, uint256 kgY) = Secp256k1.ecMul(
            k % Secp256k1.PP,
            Secp256k1.Gx,
            Secp256k1.Gy,
            Secp256k1.A,
        ...
        out_e = uint256(
            keccak256(abi.encodePacked(pubkeyX, pubkeyY, kgX, kgY, message))
        );
    ...
    function VerifyProof(
        ...
        (uint256 sgX, uint256 sgY) = Secp256k1.ecMul(
            s % Secp256k1.PP,
            Secp256k1.Gx,
            Secp256k1.Gy,
            Secp256k1.A,
        ...
        (uint256 rX, uint256 rY) = Secp256k1.ecMul(
            e,
            pubkey[0],
            pubkey[1],
            Secp256k1.A,
        ...
        (uint256 kgX, uint256 kgY) = Secp256k1.ecSub(
            sgX,
            sgY,
            rX,
            rY,
    ...
    function sign(
        bytes32 msghash,
        bytes32 privkey
    ) public pure returns (uint8, uint256, uint256) {
    ...
    function recover(
        ...
        uint256 r,
        uint256 s
    ) public pure returns (Secp256k1.Point memory) {
        uint256 x = r;
        uint256 xcubedaxb = addmod(
            addmod(
                mulmod(x, mulmod(x, x, Secp256k1.PP), Secp256k1.PP),
                Secp256k1.A * x,
                Secp256k1.PP
            ),
            Secp256k1.B,
        ...
        (uint256 Gzx, uint256 Gzy, uint256 gZz) = Secp256k1.jacMul(
            nzInv,
            G.x,
            G.y,
            1,
        ...
        (uint256 Rzx, uint256 Rzy, uint256 xZz) = Secp256k1.jacMul(
            s,
            R.x,
            R.y,
            1,
        ...
        (uint256 Qx, uint256 Qy, uint256 Qz) = Secp256k1.jacAdd(
            Gzx,
            Gzy,
            gZz,
            Rzx,
        ...
        (uint256 Qjx, uint256 Qjy, uint256 Qjz) = Secp256k1.jacMul(
            Secp256k1.invMod(x, Secp256k1.N),
            Qx,
            Qy,
            Qz,
            0,
    ...
    ) internal pure returns (Secp256k1.Point memory) {
        return
            Secp256k1.multiply(
    ...
    function ecdsaRawSign(
        ...
        uint256 s = mulmod(
            invK,
            (z + mulmod(r, uint(priv), Secp256k1.N)),
            Secp256k1.N
        );
